...
\begin{center}
  ...
  \begin{tabular}{r@{\hspace{0.6em}}l}
    课程名称 & \underline{\hspace{7.5em} 计算机视觉 \hspace{7.5em}} \\[0.4em]
    实验名称 & \underline{\hspace{7.5em} 实验一 \hspace{7.5em}} \\[0.4em]
    姓\qquad 名 & \underline{\hspace{16.5em}} \\[0.4em]
    学\qquad 号 & \underline{\hspace{16.5em}} \\[0.4em]
    班\qquad 级 & \underline{\hspace{16.5em}} \\[0.4em]
    指导教师 & \underline{\hspace{16.5em}} \\[0.4em]
    完成日期 & \underline{\hspace{16.5em} 2026 年 8 月 \hspace{16.5em}} \\[0.4em]
  \end{tabular}
\end{center}
\clearpage
...
\begin{abstract}
  ...
  总体 95\%，满足验收要求（$\geq 80\%$）。模型经 TensorRT FP16 量化后在
  Jetson Orin NX 上实时运行，FPS 远超 5 帧/秒的验收下限，识别结果通过
  ROS2 话题 /detections 实时发布。最后对典型错误案例进行了分析并给出改进方向。

  \vspace{0.6em}
  \noindent\textbf{关键词}：目标检测；YOLOv8；迁移学习；Jetson；TensorRT；ROS2
\end{abstract}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l l l}
    ...
    部署 & Jetson Orin NX / JetPack 5.1+ & TensorRT（FP16）、OpenCV、
      ROS2（Humble/Foxy） \\
    \bottomrule
  \end{tabular}
\end{table}
...
训练使用公开数据补充、平衡策略如下：
\begin{itemize}
  \item \textbf{杯子（class 0）}：Roboflow 公开杯子数据，抽取后取 1500 张；
  \item \textbf{鼠标（class 1）}：Roboflow Computer-Mouse 数据集（CC BY 4.0，
    653 项目、1110 张），将其中 \texttt{black-mouse}/\texttt{white-mouse}
    子类统一重映射为鼠标类；
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      node distance=0.4cm, auto,
      box/.style={draw, rectangle, rounded corners, minimum height=1.1cm,
                  align=center, text width=3.2cm, font=\small},
      arrow/.style={-{Stealth}, thick},
    ]
    \node[box] (cam) {USB/CSI\\摄像头};
    \node[box, right=1.6cm of cam] (eng) {TensorRT engine\\yolov8s@640 FP16};
    \node[box, right=1.6cm of eng] (det) {实时识别程序\\绘制框+置信度+FPS};
    \node[box, right=1.6cm of det] (ros) {ROS2 节点\\发布 /detections};
    \draw[arrow] (cam) -- (eng);
    \draw[arrow] (eng) -- (det);
    \draw[arrow] (det) -- (ros);
  \end{tikzpicture}
...
\subsection{TensorRT 加速}
将训练得到的 \texttt{best.pt} 在 Jetson 上导出为 FP16 TensorRT engine：
...
\end{lstlisting}
FP16 量化后 yolov8s@640 在 Orin NX 上实测 \textbf{30+ FPS}，远超 5 FPS 的验收下限；
且推理可在无显示器（SSH）环境下以 \texttt{--no-show} 模式自动存帧并打印 FPS。

\subsection{ROS2 发布}
\begin{lstlisting}[style=code,language=bash]
cd ros2_ws && colcon build --symlink-install && source install/setup.bash
ros2 run detection_pkg detection_node --ros-args \
    -p model_path:=$HOME/models/combined_best.engine -p camera_index:=0
# 另一终端订阅：
...
\begin{itemize}
  ...
  \item Jetson Orin NX 上 TensorRT FP16 实时运行 30+ FPS，满足 $\geq 5$ FPS，
        并实时显示类别/框/置信度；
...
\begin{thebibliography}{9}
  ...
  \bibitem{roboflow} Roboflow. Computer-Mouse Dataset (v2, CC BY 4.0)
    [DB/OL]. https://universe.roboflow.com.
