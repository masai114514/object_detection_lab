...
## 3. 推理引擎：优先 TensorRT，不可用则 torch FP16

> **⚠ 学校镜像的已知坑（实测）**：本实验板子的 JetPack 镜像缺
> `libnvdla_compiler.so`（`libnvinfer.so.10` 硬依赖它），导致
> `import tensorrt` 直接失败。已排查：apt 仓库无此包、清华源无 aarch64 wheel、
> NGC 源校园网不通 —— **TensorRT engine 在此镜像上导出不了**。
> 不影响验收：**torch FP16 CUDA 直推实测 24~25 FPS**，远超 ≥5 FPS 要求。
> 若老师提供了完整 JetPack SDK（或能联网 NGC），才走下面的 engine 路径。

**可选：导出 TensorRT engine（5~10 分钟，只在完整 JetPack 上做一次）**
```bash
cd ~/jetson_app/jetson_deploy
python3 scripts/export_jetson.py --model models/combined_best.pt --half
# 产出 models/combined_best.engine（仅在 import tensorrt 成功后执行）
```
- engine FP16 在 Orin NX 上 yolov8s@640 可到 30+ FPS（锦上添花，非验收必需）。
...
#   按 q：退出
#   .engine 可用时：--model models/combined_best.engine（不需要 --half）
```

**SSH 无显示**：
```bash
python3 jetson/detect.py --model models/combined_best.pt --half --no-show --save-interval 3
...
参数：`model_path`、`camera_index`、`conf`、`imgsz`、`device`（int，默认 0）、
`half`（bool，默认 false）、`topic`、`out_dir`、`show`。
`.pt` 模型建议 `-p device:=0 -p half:=true`。无显示器时自动切 headless
（只发布 + 打印 FPS，不弹窗）；SSH 无显示时若想彻底不画图，加 `-p show:=false`。
...
4. **`import tensorrt` 报 `libnvdla_compiler.so` 缺失**：学校镜像缺 DLA 编译器库，
   apt/清华/NGC 都拿不到。**别纠结 engine**，直接用 torch FP16：
   `python3 jetson/detect.py --model models/combined_best.pt --half`（实测 24~25 FPS）。
